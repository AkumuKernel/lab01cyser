import sys
import scapy.all as scapy
from nltk.corpus import words
from termcolor import colored

def decrypt_cesar(ciphertext, shift):
    plaintext = ""
    for char in ciphertext:
        if char.isalpha():
            if char.islower():
                decrypted_char = chr(((ord(char) - ord('a') - shift) % 26) + ord('a'))
            elif char.isupper():
                decrypted_char = chr(((ord(char) - ord('A') - shift) % 26) + ord('A'))
        else:
            decrypted_char = char
        plaintext += decrypted_char
    return plaintext

def find_possible_phrases(packets):
    possible_phrases = []
    for packet in packets:
        payload = packet[scapy.Raw].load[56:]
        possible_phrases.append(payload.decode())
    return possible_phrases

def is_english_word(word):
    return word.lower() in words.words()

def main():
    if len(sys.argv) != 2:
        print("Uso: python3 cesar_desencripter.py captura.pcapng")
        sys.exit(1)

    pcap_file = sys.argv[1]
    packets = scapy.rdpcap(pcap_file)

    id_to_payloads = {}
    for packet in packets:
        if scapy.ICMP in packet:
            id_icmp = packet[scapy.ICMP].id
            payload = packet[scapy.Raw].load[56:]
            if id_icmp in id_to_payloads:
                id_to_payloads[id_icmp].append(payload)
            else:
                id_to_payloads[id_icmp] = [payload]

    best_shift = None
    best_phrase = None
    best_word_count = 0

    for id_icmp, payloads in id_to_payloads.items():
        combined_payload = b''.join(payloads)
        possible_phrases = []

        # Requerimiento 3: Comprobar que se ha generado al menos una frase
        for shift in range(26):
            decrypted_text = decrypt_cesar(combined_payload.decode(), shift)
            possible_phrases.append(decrypted_text)

        # Requerimiento 4: Encontrar la frase con más sentido
        for phrase in possible_phrases:
            word_count = sum([is_english_word(word) for word in phrase.split()])
            if word_count > best_word_count:
                best_shift = possible_phrases.index(phrase)
                best_phrase = phrase
                best_word_count = word_count

    # Requerimiento 5: Imprimir todos los corrimientos
    for shift, phrase in enumerate(possible_phrases):
        if shift == best_shift:
            print(colored(f"{shift}: {phrase}", 'green'))
        else:
            print(f"{shift}: {phrase}")

if __name__ == "__main__":
    main()
